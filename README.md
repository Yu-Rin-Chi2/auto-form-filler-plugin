...
- プロフィールの JSON インポート / エクスポート（設定は含まれません）
- 日本語 / 英語 UI
...
初回はオプションページが自動で開きます（開かない場合はツールバーの拡張アイコン → ⚙ から）。「プロフィール」タブで氏名・住所・連絡先などを登録してください。**API キーの取得や設定は不要です。**
...
項目の判定には Jev（TypeSafe AI の判定特化モデル）を使います。API キーの用意は不要で、判定のリクエストは開発者が運用する中継サーバー（[`workers/`](workers/) にソースを公開しています）を経由します。利用は無料です。

中継サーバーはリクエストの内容を記録しません。過剰なアクセスを防ぐため、接続元の IP アドレスを見て回数制限のみを行います。詳細は [PRIVACY.md](PRIVACY.md) を参照してください。
