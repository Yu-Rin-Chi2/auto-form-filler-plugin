...
4. 分割グループの解決: 同じ `choice`（`phone` / `postal_code`）が DOM 順で連続する複数フィールドに付いた場合、ハイフン分割したパーツを順に割り当てる。パーツ数が合わなければ先頭フィールドに全体を入れ、残りはスキップ
   4a. 住所の分割の度合い（2026-09-23 追加。`resolve/address.ts`）: DOM 順で並んだ住所系の欄（都道府県・市区町村・番地・建物名・一体型。郵便番号は挟まってよい、それ以外の欄で区切る。漢字とカナは別々）を 2 欄以上のまとまりとして扱い、Jev の `choice` を「その欄の開始位置」として使う（一体型は直前の欄の次から）。間の抜けは前が一体型なら前、そうでなければ後ろのテキスト欄が受け持つ。都道府県・市区町村の欄が無いまとまりでは先頭のテキスト欄が都道府県から、番地以降で終わるまとまりでは末尾のテキスト欄が建物名まで受け持つ。漢字は番地と建物名の間だけ半角スペースで連結。広げた値が `maxlength` を超えればその欄自身の部分だけを入れる。このまとまりに属する欄は、手順 2 の確信度を「住所のいずれかである確率の合計」で判定する（確信度不足でスキップされる欄も範囲の手がかりには使う）
